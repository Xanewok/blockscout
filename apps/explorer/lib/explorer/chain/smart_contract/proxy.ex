defmodule Explorer.Chain.SmartContract.Proxy do
  ...
  def try_to_get_implementation_from_known_proxy_patterns(proxy_address_hash, _proxy_abi, true) do
    functions =
      [
    ...
    if implementation_address_hash_strings == [] || implementation_address_hash_strings == [burn_address_hash_string()] ||
         implementation_address_hash_strings == :error do
      fallback_proxy_detection(proxy_address_hash, nil, implementation_address_hash_strings_fallback(nil))
    else
      %{implementation_address_hash_strings: implementation_address_hash_strings, proxy_type: proxy_type}
    end
  end
  ...
  defp define_fallback_proxy_type(proxy_abi) do
    ...
    |> Enum.reduce_while(nil, fn method, acc ->
      cond do
        Map.get(method, "name") in Map.values(methods_to_proxy_types) && Map.get(method, "stateMutability") == "view" ->
